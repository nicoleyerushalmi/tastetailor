...
- **Shopping List Inefficiencies:** Manually copying ingredients from a recipe to a shopping list is tedious. Different units (e.g., cups vs. grams) cause confusion, and there is no seamless way to export these lists to third-party price-comparison bots.
...
2. **Structured AI Integration (LLM):** The ability to process dual inputs (either a structured recipe form or a free-text dish name), apply custom prompt engineering based on preferences, and return a validated JSON recipe.
...
7. **Smart Data Export System:** Formatting shopping list data into structured text, including an algorithm to handle and merge different measurement units.
...
1. **Adapt Existing Recipe:** The user submits a recipe they found elsewhere using a structured form containing mandatory fields: Recipe Title, Ingredients List, and Cooking Steps.
...
- **Categorized Selection:** A curated menu allows users to browse popular figures, such as "Gourmet Experts," "Macro-Focused Coaches," or "Plant-Based Specialists."
- **Bespoke Search:** A manual entry field enables users to input the name of any creator or culinary style they prefer.

**AI Processing & Fallback Logic:** The platform synthesizes the input (either the structured recipe or the dish name) with the user's health profile and the chosen creator's persona.
...
- The system aggregates the ingredients into the user's active shopping list in the database. It intelligently merges identical items and handles unit normalization (e.g., converting "1 cup of flour" and "100 grams of flour" into a single metric, or explicitly grouping them by standardized units to prevent duplicates).
...
4. **Smart List Merging:** The shopping list successfully demonstrates the ability to identify identical ingredients and handle basic unit discrepancies logically.
