...
export default async function NotFound() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col items-start justify-center gap-4 px-4 py-24 sm:px-6">
      ...
      <Link
        href={href}
        className="inline-flex h-11 items-center rounded-[var(--radius-control)] bg-[var(--color-accent)] px-5 text-sm font-medium text-white hover:bg-[var(--color-accent-hover)]"
      >
        {label}
      </Link>
    </main>
